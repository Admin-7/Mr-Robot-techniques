...
1. [Build a Computer from First Principles - Nand2Tetris](http://nand2tetris.org/)
    A freely available course and textbook for self-learners about the construction of a modern, full-scale computer system—hardware and software—from the ground up, which introduces many major computer science (CS) abstractions. The course follows a bottom-up path that begins with exploring how simple electrical circuits can store information (i.e., "computer memory") all the way through to the workings of a modern laptop. The lectures, book chapters and projects are modular, so you can pursue subsets of them in any desired order and scope.
